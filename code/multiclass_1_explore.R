...
# Target
tmp = cut(df.orig$SalePrice, c(-Inf,quantile(df.orig$SalePrice, c(.25,.50,.75)),Inf))
df$target = as.factor(paste0("Cat_", as.numeric(tmp)))
summary(df$target)
...
# Plot 
plots = get_plot_distr_metr_class(df, metr, color = fourcol, offset = 7, missinfo = misspct, varimpinfo = varimp)
ggsave(paste0(plotloc, "multiclass_distr_metr_final.pdf"), marrangeGrob(plots, ncol = 4, nrow = 2), 
       width = 18, height = 12)
...
ggsave(paste0(plotloc, "multiclass_corr_metr.pdf"), plot, width = 12, height = 12)
...
ggsave(paste0(plotloc, "multiclass_distr_metr_folddependency.pdf"), marrangeGrob(plots, ncol = 4, nrow = 2), 
       width = 18, height = 12)
...
# Check
plots = get_plot_distr_nomi_class(df, nomi, varimpinfo = varimp, color = fourcol, min_width = 0.9, inner_barplot = TRUE) 
ggsave(paste0(plotloc, "multiclass_distr_nomi.pdf"), marrangeGrob(plots, ncol = 4, nrow = 3, top = NULL), 
       width = 18, height = 12)
...
ggsave(paste0(plotloc, "multiclass_corr_nomi.pdf"), plot, width = 12, height = 12)
...
ggsave(paste0(plotloc, "multiclass_distr_nomi_folddependency.pdf"), marrangeGrob(plots, ncol = 4, nrow = 3), 
       width = 18, height = 12)
...
rm(df.orig)
rm(plots)
save.image("multiclass_1_explore.rdata")
