...
fit = train(df.train[predictors], df.train$target, 
            trControl = ctrl_idx_fff, metric = metric, 
            method = "ranger", 
            tuneGrid = expand.grid(mtry = seq(1,length(predictors),10),
                                   splitrule = "variance",
...
fit = train(sparse.model.matrix(formula_rightside, df.train[predictors]), df.train$target,
            ...
            tuneGrid = expand.grid(nrounds = seq(100,1100,100), num_leaves = c(10,20),
                                   learning_rate = c(0.1,0.01), feature_fraction = c(0.7),
...
perfcomp = function(method, nsim = 5) { 
  ...
  result = foreach(sim = 1:nsim, .combine = bind_rows, .packages = c("caret","Matrix","xgboost"), 
                   .export = c("df.comp","mysummary_regr","metric",
...
p  
ggsave(paste0(plotloc, "regr_model_comparison.pdf"), p, width = 12, height = 8)
...
{ 
  ...
  ctrl_idx_nopar = trainControl(method = "cv", number = 1, index = l.index, 
                                ...
                                allowParallel = FALSE,
                                summaryFunction = mysummary_regr)
  #ctrl_none = trainControl(method = "none", returnData = FALSE)
  tmp = Sys.time()
  ...
  yhat_train = predict(fit, xgb.DMatrix(sparse.model.matrix(formula_rightside, df.train[i.samp,predictors])))
  
  # Test data 
  y_test = df.test$target
  yhat_test = predict(fit, xgb.DMatrix(sparse.model.matrix(formula_rightside, df.test)))
...
p
ggsave(paste0(plotloc,"regr_learningCurve.pdf"), p, width = 8, height = 6)
