  ...
  for (sim in 1:nsim) {
    ...
    if (method == "xgbTree") { 
      fit = train(formula, data = df.train[c("target",predictors)],
                  trControl = ctrl_index, metric = metric, 
                  method = "xgbTree", 
                  tuneGrid = expand.grid(nrounds = 1000, max_depth = c(9,12), 
    ...
    if (method == "ms_boosttree") { 
      fit = train(df.train[,predictors], df.train$target, 
                  trControl = ctrl_index, metric = metric, 
                  method = ms_boosttree, 
                  tuneGrid = expand.grid(numTrees = c(1000,1500), numLeaves = 5,  
                                         learningRate = 0.1, featureFraction = 0.5,  
                                         minSplit = 20, exampleFraction = 0.5),
                  verbose = 0) 
    }
...
p  
ggsave(paste0(plotloc, "ameshousing_model_comparison.pdf"), p, width = 12, height = 8)
...
{ 
  ...
  fit = train(formula, data = df.train[i.train,c("target",predictors)],
              ...
              method = "xgbTree", 
              tuneGrid = expand.grid(nrounds = 1000, max_depth = 12, 
                                     eta = 0.01, gamma = 0, colsample_bytree = 0.5, 
                                     min_child_weight = 5, subsample = 0.5))
...
p
ggsave(paste0(plotloc,"ameshousing_learningCurve.pdf"), p, width = 8, height = 6)
