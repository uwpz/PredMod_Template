...
load("ameshousing_1_explore.rdata")
...
ggsave(paste0(plotloc, "ameshousing_performance.pdf"), marrangeGrob(plots, ncol = 2, nrow = 2, top = NULL), 
       w = 18, h = 12)
...
plots = c(suppressMessages(get_plot_distr_metr_regr(df.test, metr, target_name = "residual", ylim = NULL)), 
          get_plot_distr_nomi_regr(df.test, nomi, target_name = "residual"))
ggsave(paste0(plotloc, "ameshousing_diagnosis_residual.pdf"), marrangeGrob(plots, ncol = 4, nrow = 3), 
       width = 18, height = 12)

# Absolute residuals
summary(df.test$abs_residual)
plots = c(suppressMessages(get_plot_distr_metr_regr(df.test, metr, target_name = "abs_residual")), 
          get_plot_distr_nomi_regr(df.test, nomi, target_name = "abs_residual"))
ggsave(paste0(plotloc, "ameshousing_diagnosis_absolute_residual.pdf"), marrangeGrob(plots, ncol = 4, nrow = 3), 
       width = 18, height = 12)
...
ggsave(paste0(plotloc, "ameshousing_variable_importance.pdf"), plot, w = 8, h = 6)
...
plots = get_plot_partialdep(df.partialdep, topn_vars, df.for_partialdep = df.test, ylim = c(1e5, 3e5))
ggsave(paste0(plotloc, "ameshousing_partial_dependence.pdf"), marrangeGrob(plots, ncol = 4, nrow = 2, top = NULL), 
       w = 18, h = 12)
...
## Plot
plots = get_plot_partialdep(df.partialdep, topn_vars, df.plot_boot = df.partialdep_boot, ylim = c(1e5, 3e5))
ggsave(paste0(plotloc, "ameshousing_partial_dependence.pdf"), marrangeGrob(plots, ncol = 4, nrow = 2, top = NULL), 
       w = 18, h = 12)
...
plots = get_plot_explainer(df.plot = df.predictions[1:12,], df.values = df.model_test[1:12,], type = "regr", 
                           ylim = c(175000,250000), threshold = 5000)
ggsave(paste0(plotloc, "ameshousing_explanations.pdf"), marrangeGrob(plots, ncol = 1, nrow = 2, top = NULL), 
       w = 18, h = 12)
