...
n_maxpersample = 100000 #Take all but n_maxpersample at most
...
fit = train(df.train[predictors], df.train$target, 
            trControl = ctrl_idx_fff, metric = metric, 
            method = "ranger", 
            tuneGrid = expand.grid(mtry = c(1,7,20),
                                   splitrule = "gini",
                                   min.node.size = c(5)), 
...
fit = train(xgb.DMatrix(sparse.model.matrix(formula_rightside, df.train[predictors])), df.train$target,
            ...
            method = "xgbTree", 
            tuneGrid = expand.grid(nrounds = seq(100,2100,200), max_depth = c(6,9), 
                                   eta = c(0.01), gamma = 0, colsample_bytree = c(0.5), 
...
fit = train(df.train[predictors], df.train$target,
            ...
            tuneGrid = expand.grid(numTrees = seq(100,1100,500), numLeaves = c(10,20),
                                   learningRate = c(0.01), featureFraction = c(0.5),
                                   minSplit = c(10), exampleFraction = c(0.5)),
            verbose = 0) #!numTrees is not a sequential parameter (like in xgbTree)
plot(fit)
fit = train(sparse.model.matrix(formula_rightside, df.train[predictors]), df.train$target,
            ...
            trControl = ctrl_idx_fff, metric = metric, 
            method = lgbm, 
            tuneGrid = expand.grid(nrounds = seq(100,2100,100), num_leaves = c(10,20),
                                   learning_rate = c(0.01), feature_fraction = c(0.5),
                                   min_data_in_leaf = c(10), bagging_fraction = c(0.5)),
            max_depth = 3,
            verbose = 0) 
plot(fit)
                               ...
                               "formula","formula_binned","formula_rightside","formula_binned_rightside")) %dopar% 
  {
    ...
    if (method == "xgbTree") { 
      fit = train(xgb.DMatrix(sparse.model.matrix(formula_rightside, df.train[predictors])), df.train$target,
                  ...
                  method = "xgbTree", 
                  tuneGrid = expand.grid(nrounds = seq(100,2100,200), max_depth = 9, 
...
df.lc = foreach(i = 1:2, .combine = bind_rows, .packages = c("caret","xgboost")) %do% #NO dopar for xgboost!
{ 
  #i = 1
